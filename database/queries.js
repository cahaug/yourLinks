...
module.exports = { 
    ...
    getEntries2(userId){
        return knex('entries').where("userId", userId).orderBy('entryId', 'asc').leftJoin('stats', 'entries.entryId', 'stats.entryId').select('stats.entryId').count().groupBy('entries.entryId').orderBy('stats.entryId', 'asc');
    },
